...
(
 ...
 select f_mrvipnumber,sum(f_price) f_price from junya_project.stg_flipos_t_order_da 
    where pt='20211008'
...
------------------------------  消费剔除拼单 ------------------------------ 
  select
     ...
     from 
       junya_project. dwd_junya_t_order_da t1
     left join (select * from junya_project. dwd_junya_t_refund_order_da where pt='${bizdate}') t2 on t1.f_id=t2.order_id
     where 
        t1.pt='${bizdate}'
        and t1.type!=1
        and t1.pos_type=3
...
------------------------------  消费拼单数据 ------------------------------ 
  select
     ...
     from 
            junya_project.dwd_junya_t_order_da t1
     left join (select * from  junya_project.dwd_junya_t_order_group_user_da where pt='${bizdate}') t2 on t1.group_buy_no=t2.group_buy_no
     where 
        t1.pt='${bizdate}'
        and t1.type=1
        and t1.pos_type=3
...
-------------------------------------------------- 有积点发放 -------------------------------------------------- 
    ...
    select
        ...
        from    junya_project.dwd_junya_t_member_point_trans_da t1
            left join (select * from junya_project.dwd_junya_t_member_vip_account_da where pt='${bizdate}') t2 on t1.f_accountid=t2.f_id
...
FROM
        junya_project.dwd_junya_t_mr_vip_recharge_da t1 
...
inner join (
    ...
    select 
        ...
        from    junya_project.dwd_junya_t_member_vip_da t1
    where t1.pt='${bizdate}' 
